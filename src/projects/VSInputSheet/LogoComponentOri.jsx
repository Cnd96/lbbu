...
const LogoComponent =props=> {
  ...
  const addStyleButtonClicked=(styleNo)=>{
    ...
    if(addedStyle){
      addedStyle.wareHouse=wareHouse?wareHouse.name:"";
  ...
  const changeSelectedStyleNo=(styleNo)=>{
    setSelectedStyle(styleNo);
    const selectedStylesData=stylesData.find(l=>l.styleNo==styleNo);
  ...
  return(
    <div className="container">
      <Grid container spacing={1} style={{marginTop:"2vw"}}>
        ...
        <button className="btn btn-primary" hidden={!showStyle} onClick={onInputSheetDownload}>Download</button>
        </Grid>
      </Grid>
        ...
        :<h2></h2>}
        {isOtrUploaded?
         <div  style={{marginTop:"2vw",width:"21vw"}}>
           <select className="form-control form-control-sm"
             value={selectedStyle}
             onChange={(e)=>{
              changeSelectedStyleNo(e.target.value);
              }}
             >
             <option value="" defaultValue disabled>Select Style</option>
               {stylesData.map(({styleNo}) =>
                 <option value={styleNo} key={styleNo}>{styleNo}</option>
               )}
           </select>
         </div>:<div></div>
        }
